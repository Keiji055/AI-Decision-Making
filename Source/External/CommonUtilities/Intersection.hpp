...
namespace CommonUtilities
{
	...
	// Returns true if an AABB is colliding with another AABB in 2D
	template<class T>
	inline bool IntersectionAABBAABB2D(const AABB2D<T>& aAABB1, const AABB2D<T>& aAABB2)
	{
		Vector2<T> aMax = aAABB1.GetMax();
		Vector2<T> aMin = aAABB1.GetMin();
		Vector2<T> bMax = aAABB2.GetMax();
		Vector2<T> bMin = aAABB2.GetMin();

		return  !(aMax.x < bMin.x || aMin.x > bMax.x) &&  // To the left or right
			!(aMax.y < bMin.y || aMin.y > bMax.y);    // Above or below
	}

	// Returns true if a Ray is intersecting with an AABB in 2D
	template<class T>
	inline bool IntersectionRayAABB2D(const Ray2D<T>& aRay, const AABB2D<T>& aBox)
	{
		Vector2<T> invDir(1 / aRay.GetDirection().x, 1 / aRay.GetDirection().y);

		Vector2<T> tMinMinus = (aBox.GetMin() - aRay.GetOrigin());
		Vector2<T> tMaxMinus = (aBox.GetMax() - aRay.GetOrigin());

		Vector2<T> tMin = { tMinMinus.x * invDir.x, tMinMinus.y * invDir.y };
		Vector2<T> tMax = { tMaxMinus.x * invDir.x , tMaxMinus.y * invDir.y };

		// Apply min and max to individual components (x and y)
		Vector2<T> t1 = { std::min(tMin.x, tMax.x), std::min(tMin.y, tMax.y) };  // Min of each axis
		Vector2<T> t2 = { std::max(tMin.x, tMax.x), std::max(tMin.y, tMax.y) };  // Max of each axis

		T tEnter = std::max(t1.x, t1.y);  // The largest tEnter (x, y)
		T tExit = std::min(t2.x, t2.y);   // The smallest tExit (x, y)

		// If tEnter is greater than tExit, the ray doesn't intersect the box
	...
	// Returns true if a finite-length Ray is intersecting with an AABB in 2D
	template<class T>
	inline bool IntersectionRayAABB2D(const Ray2D<T>& aRay, const AABB2D<T>& aBox, T aRayLength)
	{
		Vector2<T> invDir(1 / aRay.GetDirection().x, 1 / aRay.GetDirection().y);
		...
		// If tExit is less than zero, the intersection is behind the ray's origin
		if (tExit < 0) {
			return false;
		}

		// If tEnter is greater than the ray's length, the intersection is beyond the ray's finite length
		if (tEnter > aRayLength) {
			return false;
		}

		// The intersection occurs between tEnter and tExit, within the ray's length
		return true;
	}
